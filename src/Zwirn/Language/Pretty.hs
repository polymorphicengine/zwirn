...
instance Pretty Term where
  ...
  ppr p (TCase t Nothing ts) = text "case" <+> ppr p t <+> text "of" <+> vcat (punctuate (text ";") (map (\(l, x) -> ppr p l <+> text "->" <+> ppr p x) ts))
  ppr p (TCase t (Just def) ts) = text "case" <+> ppr p t <+> text "of" <+> vcat (punctuate (text ";") (map (\(l, x) -> ppr p l <+> text "->" <+> ppr p x) ts ++ [text "_ ->" <+> ppr p def]))
